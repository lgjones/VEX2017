...
#pragma config(Motor,  port4,           right_drive_1, tmotorVex393_MC29, PIDControl, driveRight, encoderPort, I2C_2)
...
#pragma config(Motor,  port10,          strafe,        tmotorVex393_HBridge, openLoop)
...
//void turnToAngle(float rel_angle);
void driveDist(int encodons, int speed);
void latch();
void unlatch();
float map360(float ang);
void turnDistance(int encodons);
void moveShooterAuto(int encodons);
...
/*void turnToAngle(float rel_angle) { //turn angle degrees from the current position (relative angle)
	int dir = 1;
	const float K_P = 2.0; //TUNING PARAMETER
	start_angle = map360(SensorValue(angle)*(360/2900.)); //-4
	current_angle = start_angle;

	relative_start_angle = map360(current_angle - start_angle);

	if(rel_angle < 0) {
		dir = -1;
	}

	target = map360(start_angle + rel_angle);

	while(relative_start_angle < map360(target - 3) || relative_start_angle > map360(target + 3)) { //while the angle isn't within +/- 3 degrees of what is desired
		int pwr = dir*K_P*(rel_angle - (current_angle - start_angle));
		if(pwr > 30) {
			pwr = 30;
		}
		if(pwr < -30) {
		...
		current_angle = map360(SensorValue(angle)*(360/2900.)); //-4
		relative_start_angle = map360(current_angle - start_angle);
	}
...
void driveDist(int encodons, int speed) {
	resetMotorEncoder(left_drive_1);
	resetMotorEncoder(right_drive_1);
	setMotorTarget(right_drive_1, encodons, speed, false);
	setMotorTarget(left_drive_1, -encodons, speed, false);
}

void moveShooterAuto(int encodons) {
  setMotorTarget(right_launch, -encodons, 115, true);
  setMotorTarget(left_launch, encodons, 115, true);
}
...
void turnDistance(int encodons) {
	resetMotorEncoder(left_drive_1);
	resetMotorEncoder(right_drive_1);
	setMotorTarget(right_drive_1, encodons, 40, false);
	setMotorTarget(left_drive_1, encodons, 40, false);
}
